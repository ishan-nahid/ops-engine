...
function json(data: unknown, init: ResponseInit = {}): Response {
  return new Response(JSON.stringify(data, null, 2), { ...init, headers: { ...jsonHeaders, ...(init.headers || {}) } });
}
...
function summarizePayload(payload: AgentPayload): { status: Status; summary: Record<string, unknown> } {
  ...
  return {
    ...
    summary: {
      ...
      total_5xx: total5xx,
      branch: asRecord(payload.meta).git ? asRecord(asRecord(payload.meta).git).branch : null,
      sha: asRecord(payload.meta).git ? asRecord(asRecord(payload.meta).git).sha : null,
    },
  };
}
...
async function sendAlert(env: Env, text: string): Promise<void> {
  ...
  try {
    ...
    // Alerting must never break monitoring writes.
  }
}
...
async function createOrKeepIncident(env: Env, status: Status, source: string, summary: Record<string, unknown>): Promise<void> {
  ...
  await env.DB.prepare("INSERT INTO incidents (id, title, severity, status, source, started_at, summary, metadata_json) VALUES (?, ?, ?, 'open', ?, ?, ?, ?)")
    .bind(id, title, status, source, nowIso(), JSON.stringify(summary), JSON.stringify({ created_by: "ops-engine" })).run();
...
async function resolveOpenIncidents(env: Env, source: string, note: string): Promise<void> {
  const rows = await env.DB.prepare("SELECT id, title FROM incidents WHERE status = 'open' AND source = ?").bind(source).all<{ id: string; title: string }>();
...
async function recordApiTraffic(env: Env, heartbeatId: string, source: string, collectedAt: string, raw: Record<string, unknown>): Promise<void> {
  ...
  await env.DB.prepare("INSERT INTO api_traffic_summaries (id, heartbeat_id, source, collected_at, window_seconds, total_requests, total_2xx, total_3xx, total_4xx, total_5xx, top_paths_json, status_codes_json, methods_json, slow_paths_json, privacy_mode, metadata_json) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)")
    .bind(
      uuid(), heartbeatId, source, collectedAt,
      Number(raw.window_seconds || 3600), Number(raw.total_requests || 0), Number(raw.total_2xx || 0), Number(raw.total_3xx || 0), Number(raw.total_4xx || 0), Number(raw.total_5xx || 0),
      JSON.stringify(raw.top_paths || []), JSON.stringify(raw.status_codes || {}), JSON.stringify(raw.methods || {}), JSON.stringify(raw.slow_paths || []),
      String(raw.privacy_mode || "aggregate"), JSON.stringify(raw.metadata || {}),
    ).run();
}
...
async function handleLatest(env: Env): Promise<Response> {
  ...
  return json({ heartbeat, uptime: uptime.results || [], incidents: incidents.results || [], deploy, server, api_traffic: apiTraffic, backup });
}
...
async function checkTarget(env: Env, targetKey: string, targetUrl: string): Promise<void> {
  ...
  try {
    const headers: Record<string, string> = { "user-agent": "ops-engine/0.2" };
...
async function route(request: Request, env: Env): Promise<Response> {
  ...
  if (path === "/" || path === "/api") return json({ name: env.APP_NAME || "Ops Engine", status: "ok", version: "0.2" });
