...
Privacy note: API/request telemetry is aggregate or hashed. This agent never
collects request bodies, cookies, auth headers, raw IPs, or raw user IDs.
"""
...
def run_cmd(cmd: list[str], timeout: int = 8, cwd: str | None = None, extra_env: dict[str, str] | None = None) -> tuple[int, str, str]:
    try:
        env_vars = os.environ.copy()
        if extra_env:
            env_vars.update(extra_env)
        proc = subprocess.run(cmd, capture_output=True, text=True, timeout=timeout, check=False, cwd=cwd, env=env_vars)
...
def collect_smw_health() -> dict[str, Any]:
    ...
    try:
        response = requests.get(url, headers=headers, timeout=env_int("HTTP_TIMEOUT_SECONDS", 8))
...
def collect_business(smw_health: dict[str, Any]) -> dict[str, Any]:
    ...
    counters = data.get("business_counters") if isinstance(data, dict) and isinstance(data.get("business_counters"), dict) else {}
    return {"status": "live" if counters else "missing", "source": "smw-health-summary", "pending_admissions": counters.get("pending_admissions"), "payments": counters.get("payments"), "users": counters.get("users"), "raw_counters": counters}


def collect_backups() -> dict[str, Any]:
    raw_dirs = env("BACKUP_DIRS") or env("BACKUP_DIR")
    folders = [Path(x.strip()).expanduser() for x in raw_dirs.split(",") if x.strip()] or [Path("/home/ishan/db_backups"), Path("/home/ishan/backups"), Path("/home/ishan/backup"), Path("/var/backups"), Path("/home/ishan/log_exports")]
...
def collect_git() -> dict[str, Any]:
    ...
    _, branch, _ = run_cmd(["git", "branch", "--show-current"], timeout=5, cwd=str(cwd))
    _, dirty, _ = run_cmd(["git", "status", "--porcelain"], timeout=5, cwd=str(cwd))
    _, msg, _ = run_cmd(["git", "log", "-1", "--pretty=%s"], timeout=5, cwd=str(cwd))
    return {"status": "live", "sha": sha, "short_sha": sha[:12], "branch": branch or None, "is_dirty": bool(dirty), "last_commit_message": msg or None}


def collect_deployment(git_info: dict[str, Any]) -> dict[str, Any]:
    return {"status": "live" if git_info.get("sha") else "missing", "git": git_info, "current_branch": git_info.get("branch"), "current_sha": git_info.get("sha"), "short_sha": git_info.get("short_sha"), "is_dirty": git_info.get("is_dirty"), "last_commit_message": git_info.get("last_commit_message")}


def psql_query(sql: str, timeout: int = 8) -> tuple[bool, list[dict[str, Any]] | str]:
    cmd = [
        "psql", "-X", "-q", "-t", "-A", "-F", "\t",
        "-h", env("POSTGRES_HOST", env("DBHOST", "127.0.0.1")),
        "-p", env("POSTGRES_PORT", env("DBPORT", "5432")),
        "-U", env("POSTGRES_USER", env("DBUSER", "postgres")),
        "-d", env("POSTGRES_DB", env("DBNAME", "smw_db")),
        "-c", sql,
    ]
    extra = {}
    ...
    code, stdout, stderr = run_cmd(cmd, timeout=timeout, extra_env=extra)
    if code != 0:
...
def collect_database() -> dict[str, Any]:
    ...
    SELECT json_build_object(
      ...
      'lock_waiting_queries', (SELECT count(*) FROM pg_stat_activity WHERE wait_event_type = 'Lock'),
      'sessions_with_wait_event', (SELECT count(*) FROM pg_stat_activity WHERE wait_event IS NOT NULL),
...
def collect_queue() -> dict[str, Any]:
    ...
    return {"status": "live", "queues": queues, "total_depth": total_depth, "worker_service": service_status(env("CELERY_WORKER_SERVICE", "celery_smw")), "beat_service": service_status(env("CELERY_BEAT_SERVICE", "celery-beat-smw"))}
...
def _safe_path(raw_path: str) -> str:
    ...
    cleaned: list[str] = []
    for part in [p for p in path.split("/") if p][:6]:
        cleaned.append(":id" if part.isdigit() or re.fullmatch(r"[0-9a-fA-F-]{16,}", part) else part[:60])
...
def collect_api_traffic() -> dict[str, Any]:
    ...
    paths: Counter[str] = Counter(); status_codes: Counter[str] = Counter(); methods: Counter[str] = Counter(); slow: list[dict[str, Any]] = []
    for line in lines:
        match = API_LOG_RE.search(line)
        if not match:
            continue
        total += 1
        method, status_raw, duration = match.group("method"), match.group("status"), int(match.group("duration"))
        status = int(status_raw) if status_raw.isdigit() else 0
        safe_path = _safe_path(match.group("path"))
        paths[safe_path] += 1; methods[method] += 1; status_codes[str(status)] += 1
        if 200 <= status < 300: total_2xx += 1
        elif 300 <= status < 400: total_3xx += 1
        elif 400 <= status < 500: total_4xx += 1
        elif status >= 500: total_5xx += 1
    ...
    return {"privacy_mode": "aggregate", "window_seconds": 3600, "total_requests": total, "total_2xx": total_2xx, "total_3xx": total_3xx, "total_4xx": total_4xx, "total_5xx": total_5xx, "top_paths": [{"path": p, "count": c} for p, c in paths.most_common(12)], "status_codes": dict(status_codes), "methods": dict(methods), "slow_paths": slow[:10], "metadata": {"source": "journalctl", "unit": env("API_LOG_UNIT", "gunicorn-smw")}}


def _relative_project_path(path: str) -> str:
    project_dir = env("SMW_PROJECT_DIR", "/var/www/html/SMW-v1")
    backend_dir = str(Path(project_dir) / "Backend")
    for prefix in [backend_dir, project_dir]:
...
def _clean_trace_line(line: str) -> str:
    return re.sub(r"^[A-Z][a-z]{2}\s+\d+\s+[0-9:]+\s+\S+\s+[^:]+:\s*", "", line).strip()


def _extract_trace_group(unit: str, block: list[str]) -> dict[str, Any] | None:
    cleaned = [_clean_trace_line(x) for x in block if x.strip()]
    if not cleaned:
        return None
    traceback = "\n".join(cleaned)[-6000:]
    frames: list[dict[str, Any]] = []
    for idx, line in enumerate(cleaned):
        match = TRACE_FRAME_RE.search(line)
        if not match:
            continue
        code_context = cleaned[idx + 1].strip() if idx + 1 < len(cleaned) and not TRACE_FRAME_RE.search(cleaned[idx + 1]) else None
        frames.append({"file": match.group("file"), "line": int(match.group("line")), "function": match.group("function"), "code_context": code_context})
    exc_class = "LogError"; exc_message = cleaned[-1]
    for line in reversed(cleaned):
        match = EXCEPTION_RE.search(line)
        if match:
            exc_class = match.group("class").split(".")[-1]
            exc_message = match.group("message") or line
            break
    app_frame = None
    project_dir = env("SMW_PROJECT_DIR", "/var/www/html/SMW-v1")
    for frame in reversed(frames):
        file_path = str(frame.get("file", ""))
        if project_dir in file_path and "site-packages" not in file_path:
            app_frame = frame
            break
    if not app_frame and frames:
        app_frame = frames[-1]
    rel_file = _relative_project_path(str(app_frame.get("file"))) if app_frame else None
    line_no = app_frame.get("line") if app_frame else None
    function = app_frame.get("function") if app_frame else None
    code_context = app_frame.get("code_context") if app_frame else None
    path_match = re.search(r"(/api/\S+)", traceback)
    request_id_match = RID_RE.search(traceback)
    fingerprint = f"{unit}:{exc_class}:{rel_file or '-'}:{line_no or '-'}"
    return {"fingerprint": fingerprint, "message": str(exc_message)[-1000:], "traceback": traceback, "request_id": request_id_match.group("rid") if request_id_match else None, "path": _safe_path(path_match.group(1)) if path_match else None, "severity": "error", "unit": unit, "exception_class": exc_class, "file": rel_file, "line": line_no, "function": function, "code_context": code_context, "frames": [{**f, "file": _relative_project_path(str(f.get("file")))} for f in frames[-12:]]}


def collect_errors() -> dict[str, Any]:
    checks: dict[str, Any] = {}; groups: Counter[str] = Counter(); samples: dict[str, dict[str, Any]] = {}
    units = [x.strip() for x in env("ERROR_LOG_UNITS", "gunicorn-smw,celery_smw,nginx").split(",") if x.strip()]
    for unit in units:
        ...
        starts = [i for i, line in enumerate(lines) if "Traceback (most recent call last)" in line or "ERROR" in line or "CRITICAL" in line]
        if not starts and lines:
            starts = [max(0, len(lines) - 20)]
        for start in starts[-25:]:
            block = lines[start:min(start + 80, len(lines))]
            parsed = _extract_trace_group(unit, block)
            if parsed:
...
def collect_user_experience() -> dict[str, Any]:
    ...
    count = js_errors = 0; lcp: list[float] = []; cls: list[float] = []; inp: list[float] = []
    for line in lines:
        try: item = json.loads(line)
        except Exception: continue
        count += 1; kind = item.get("kind"); value = item.get("value")
...
def _state_file() -> Path:
    return Path(env("OPS_ENGINE_AGENT_STATE_FILE", "/usr/local/ops-engine-agent/state.json"))
...
def _save_state(state: dict[str, Any]) -> None:
    try:
        path = _state_file(); path.parent.mkdir(parents=True, exist_ok=True); tmp = path.with_suffix(".tmp"); tmp.write_text(json.dumps(state, sort_keys=True)); tmp.replace(path)
    except Exception:
        pass
...
def collect_request_events() -> list[dict[str, Any]]:
    ...
    state = _load_state(); state_key = f"request_events:{path}"; file_size = path.stat().st_size
    last_offset = int(state.get(state_key, 0) or 0)
    if last_offset < 0 or last_offset > file_size:
        last_offset = 0
    try:
        with path.open("rb") as fh:
            fh.seek(last_offset); raw = fh.read().decode("utf-8", errors="replace"); new_offset = fh.tell()
    except Exception:
        return []
    events: list[dict[str, Any]] = []; seen_ids: set[str] = set(); cutoff = time.time() - env_int("REQUEST_EVENTS_WINDOW_SECONDS", 3600)
    for line in raw.splitlines():
        try: item = json.loads(line)
        except Exception: continue
        if not isinstance(item, dict): continue
        try:
            if datetime.fromisoformat(str(item.get("ts", "")).replace("Z", "+00:00")).timestamp() < cutoff: continue
        except Exception:
            pass
...
def push(payload: dict[str, Any]) -> None:
    url = env("OPS_ENGINE_API_URL"); token = env("OPS_ENGINE_AGENT_TOKEN")
    if not url or not token:
        raise SystemExit("OPS_ENGINE_API_URL and OPS_ENGINE_AGENT_TOKEN are required")
    response = requests.post(url, headers={"authorization": f"Bearer {token}", "content-type": "application/json"}, json=payload, timeout=env_int("HTTP_TIMEOUT_SECONDS", 8))
